/**
 * H.264 Video Decoder for KVM Relay Client
 * 
...
class H264Decoder {
     ...
     * @param {Uint8Array} frameData - Raw H.264 NAL units with optional container header
    ...
    async decode(frameData, metadata = {}) {
        ...
        try {
            // Check for H.264 container header
            let nalData = frameData;
            let isKeyframe = metadata.isKeyframe || false;
            let timestamp = metadata.timestamp || Date.now() * 1000;
            
            if (frameData.length > 21) {
                const magic = String.fromCharCode(frameData[0], frameData[1], frameData[2], frameData[3]);
                if (magic === 'H264') {
                    // Parse container header
                    const view = new DataView(frameData.buffer, frameData.byteOffset);
            ...
            if (this.useWebCodecs) {
                await this.decodeWithWebCodecs(nalData, { isKeyframe, timestamp });
            } else {
                await this.decodeWithSoftware(nalData, { isKeyframe, timestamp });
            }
    ...
    /**
     * Extract pixel data from H.264 stream
     */
